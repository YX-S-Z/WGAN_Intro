...
Where $$\gamma$$ is the set of all possible joint probability distribution on $$\mathcal{X}\times\mathcal{X}$$ that have marginal on $$\mathbb{P}$$ and $$\mathbb{Q}$$. Don't be scared by this definition, this definition can be interpreted in this way: image the supports of probability distributions $$\mathcal{P}_r$$ and $$\mathcal{P}_g$$ are $$\mathcal{M}$$ and $$\mathcal{P}$$, we want to move all the points from one manifold $$\mathcal{P}$$ to construct the manifold $$\mathcal{M}$$, and Wasserstein metric represents the __minimum__ total travel distance we have to move for __all the points__.
